...
public class Start {
  ...
  public void start() { //create a public but not static way to start the program
    while (true) { //do an infinite loop
      ...
      var maybeFinalCalc = this.calculate(new StringCharactorIterator(input)); //make a charactorIterator to pass
      if (!maybeFinalCalc.isPresent() { //checks if not present
        System.out.println("not a valid expresion"); //tells user expresion was not valid
        continue; //pass to next iteratation
      }
      System.out.println(input.get()) //print the result if seccesfull (bad spelling)
    }
  }
  private Optional<String> getInput() {
    ...
    return Scanner.nextLine(); //theres never an exception now and the caller must handle case with no string
  }
  private Optional<String> calculate(CharactorIterator expr) { //take an iterator to allow recursiveness
